...
#Checks for keypasses or window close.
while True:
    clock.tick(30)
...
#Changes from one window of scenery to the next.
    if map_position >=640 and activateMap == 0:
        activeMap = 1
        map0 = create_map(20, 29)
        obstacles = updateObstacleGroup(map0, map1)
        if map_position >=1280 and activateMap == 1:
            activeMap = 0
            for ob in map0:
                ob.location[1] = ob.location[1] - 1280
            map_position = map+position - 1280
            map1 = create_map(10, 19)
            obstacles = updateObstacleGroup(map0, map1)

    for obstacle in obstacles:
        obstalce.scrll(map_position)
